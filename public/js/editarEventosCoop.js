/**
 * Editor visual de eventos_online.xlsx (editarEventosCoop.html).
 */
(function () {
    'use strict';

    const M = window.DCEditarEventosCoopModel;
    if (!M) {
        document.body.innerHTML = '<p style="color:#fff;padding:2rem">Falta editarEventosCoopModel.js</p>';
        return;
    }

    const $ = (id) => document.getElementById(id);
    const toolbar    = $('editar-eventos-coop-toolbar');
    const aviso      = $('editar-eventos-coop-aviso');
    const layout     = $('editar-eventos-coop-layout');
    const listaEl    = $('editar-eventos-coop-lista');
    const formEl     = $('editar-eventos-coop-form');
    const erroresEl  = $('editar-eventos-coop-errores');
    const contadorEl = $('editar-eventos-coop-contador');
    ...
    let state = {
        columnas: M.COLUMNAS_EVENTO_COOP.slice(),
        filas: [],
        filasAlCargar: 0,
        selIndex: -1,
        dirty: false,
    ...
    async function api(path, opts = {}) {
        ...
        if (!res.ok) {
            if (res.status === 404 && String(path).includes('/api/eventos-online-editor')) {
                const err = new Error('EVENTOS_ONLINE_EDITOR_API_MISSING');
                err.code = 'EVENTOS_ONLINE_EDITOR_API_MISSING';
                throw err;
            }
    ...
    async function confirmarDescartar() {
        ...
        return window.confirm('Hay cambios sin guardar en eventos_online.xlsx. ¿Descartarlos?');
    }

    function indicesFiltrados() {
        const q = $('filtro-nombre-evento-coop')?.value || '';
    ...
    function initTableroPicker() {
        const dialog = $('editar-eventos-coop-dialog-tablero');
        const grid   = $('editar-eventos-coop-tablero-grid');
        const filtro = $('editar-eventos-coop-tablero-filtro');
        const selLabel = $('editar-eventos-coop-tablero-seleccion');
        ...
        function crearCard(nombre, esVacio) {
            ...
            const media = document.createElement('div'); media.className = 'crear-ep-recurso-card-media';
        ...
        $('editar-eventos-coop-tablero-aceptar')?.addEventListener('click', confirmar);
        $('editar-eventos-coop-tablero-cancelar')?.addEventListener('click', () => { dialog.close(); tableroPickerSession = null; });
        dialog.addEventListener('cancel', () => { tableroPickerSession = null; });

        window._abrirTableroPickerEventosCoop = function (opts) {
    ...
    function fieldTablero(label, value, onChange) {
        let current = value ?? '';
        const wrap = document.createElement('div'); wrap.className = 'crear-ep-field';
        const lab = document.createElement('label'); lab.textContent = label;
        const trigger = document.createElement('button'); trigger.type = 'button'; trigger.className = 'crear-ep-recurso-trigger';
        const thumb = document.createElement('img'); thumb.className = 'crear-ep-recurso-trigger-thumb'; thumb.alt = '';
        ...
        trigger.addEventListener('click', () => {
            window._abrirTableroPickerEventosCoop?.({ value: current, onAccept: (v) => { refresh(v); onChange(v); } });
        });
    ...
    function pintarSlot(slotEl, nombreCarta, etiquetaVacio, esBoss) {
        ...
        if (!nom) {
            ...
            slotEl.appendChild(vacio); return;
        }
    ...
    function renderForm() {
        ...
        basica.appendChild(fieldInput('ID_evento_online', fila.ID_evento_online, (v) => {
            fila.ID_evento_online = Number(v) || 0; marcarDirty(); renderLista();
        }, { type: 'number' }));
        ...
        enemigos.innerHTML = '<h3>Enemigos del evento (8 normales + boss)</h3>';
        ...
        const del = btn('Eliminar este evento', 'crear-ep-btn--peligro', () => {
            if (!window.confirm(`¿Eliminar evento #${fila.ID_evento_online} «${fila.nombre}»?`)) return;
    ...
    function renderErroresFila() {
        ...
        const errs = M.validarFilaEventoCoop(fila, state.selIndex, state.filas, M.nombresCartasEnCatalogoSet(filasCatalogoCartas), skinsIndexadosCache);
    ...
    function renderLista() {
        ...
        indices.forEach((i) => {
            ...
            const li = document.createElement('li'); li.className = 'editar-desafios-lista-item';
            const b = document.createElement('button'); b.type = 'button';
            b.className = 'editar-desafios-lista-btn' + (i === state.selIndex ? ' editar-desafios-lista-btn--sel' : '');
            b.innerHTML = `<strong>#${fila.ID_evento_online} ${fila.nombre || '(sin nombre)'}</strong>`;
            b.addEventListener('click', () => { state.selIndex = i; renderAll(); });
            li.appendChild(b); listaEl.appendChild(li);
        });
    ...
    function renderToolbar() {
        ...
        span.innerHTML = `<strong>eventos_online.xlsx</strong>${state.dirty ? ' · sin guardar' : ''}`;
    ...
    async function cargarCatalogo(force) {
        ...
        try {
            const data = await api('/api/eventos-online-editor/catalogo');
            ...
            renderAll(); toastMsg('Catálogo de eventos coop cargado.');
    ...
    async function guardarCatalogo() {
        ...
        try {
            const guardado = await window.DCEditorGuardadoSeguro.intentarGuardarCatalogo({
                ...
                campoClave: 'nombre',
                minAbsoluto: 1,
                etiquetaRecurso: 'eventos_online.xlsx',
                guardarFn: ({ confirmarTruncamiento }) => api('/api/eventos-online-editor/catalogo', {
                    method: 'PUT',
                    body: JSON.stringify({
            ...
            window.DCEditorSessionLog?.registrarGuardado?.('eventosCoop', 'Guardado eventos_online.xlsx', ['public/resources/eventos_online.xlsx']);
            toastMsg('eventos_online.xlsx guardado correctamente.');
        } catch (e) { toastMsg(e.message || 'Error al guardar', true); }
    }

    function nuevaFila() {
        const fila = M.crearFilaEventoCoopVacia();
        fila.ID_evento_online = M.siguienteId(state.filas);
        fila.nombre = `Nuevo_evento_online_${fila.ID_evento_online}`;
    ...
    function mostrarAvisoInhabilitado(err) {
        ...
        if (!aviso) return;
        aviso.hidden = false;
        if (err?.code === 'EVENTOS_ONLINE_EDITOR_API_MISSING') {
            aviso.innerHTML = 'API del editor no disponible. Despliega la rama <code>dev</code> con las rutas <code>/api/eventos-online-editor/*</code>.';
        } else {
    ...
    async function init() {
        $('editar-eventos-coop-nuevo')?.addEventListener('click', nuevaFila);
        $('filtro-nombre-evento-coop')?.addEventListener('input', () => renderLista());
        $('filtro-nombre-evento-coop')?.addEventListener('change', () => renderLista());
