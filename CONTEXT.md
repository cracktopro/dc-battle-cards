...
| `DCSkinsCartas` | `js/skinsCartas.js` | Parent card, skins, resolver catálogo; notación Excel `Parent[skin_id]` (p. ej. `Cyborg[0]`); **`resolverCartaEnemigoVistaSync`** (paneles evento/desafío/coop con caché de skins) |
...
- **Apariencias en enemigos/recompensas:** celdas `enemigo1`…`boss` y sorteo 80/20 de recompensa admiten `Parent[skin_id]` (p. ej. `Cyborg[0]`). Paneles: `asegurarSkinsCargados` antes del render + `resolverCartaEnemigoVistaSync`; combate: `construirEstadoDesafio` → `resolverFilaCatalogoConSkin`; victoria: `otorgarRecompensasDesafio` desbloquea apariencia vía `construirRecompensaSkinDesdeReferencia` + `persistirSkinsRecompensaEnUsuario` (igual que asaltos).
...
- **Modal de recompensas coop:** mismo patrón que PvE offline — `otorgarRecompensasCoop({ soloLocal: true })` pinta la UI al instante; Firebase y misiones (`online`, `boss`, colección) en segundo plano. Precarga de catálogo/skins al iniciar `inicializarPartidaCoopUi` (`coopPrecargarRecursosRecompensas`). El fin de partida emite `multiplayer:coop:resultado` y **cada cliente** (P1 y P2) aplica recompensas al recibir `dc:coop-resultado` (una sola vez; ya no se llama `mostrarFin` en local y en red).
...
- **Paneles en multijugador (`multijugadorEventosCoop.js`):** primer render solo espera `eventos_online.xlsx` + catálogo (`DCCatalogoCartas`); skins y refresco de usuario van en segundo plano con **re-render** al terminar (apariencias en carruseles vía `resolverCartaEnemigoVistaSync`). Carruseles 3D de enemigos se montan por lotes (`requestAnimationFrame`, 2 por frame) con miniatura estática previa (`DCEventoPanelUi` + `encolarCarrusel`). Combate servidor: `construirMazoBotCoopDesdeEvento` → `resolverFilaCatalogoConSkinServidor`. Recompensa coop: `otorgarRecompensasCoop` trata `Parent[n]` en enemigos/boss sorteados igual que eventos offline.
...
- Engine: `js/episodio-engine.js`, timeline lineal **por capítulo**:
  ...
  - `combate`: `cartas_jugador[]`, `nivel_jugador`, `cartas_BOT[]`, `nivel_BOT`, **`tablero`** (opcional, nombre de archivo en `resources/tableros/`, p. ej. `"tablero_background_gotham"` — misma convención que eventos/asaltos en Excel). **`cartas_BOT[]` y `cartas_jugador[]` admiten `Parent[skin_id]`** (p. ej. `Cyborg[0]`): previa vía `DCEpisodiosRequisitos.construirCartasEnriquecidasDesdeEntradas` + `resolverFilaCatalogoConSkin`; combate en `partida.js` → `construirMazoEpisodioDesdeNombres`. **`nivel_jugador: 0`** → cada carta del jugador usa el **nivel de su copia en colección** (`DCEpisodiosRequisitos.construirEntradasMazoJugadorEpisodio`). Cualquier otro valor fija ese nivel para todas (como antes). Previa de combate: miniaturas `carta-mini` completas (poder, salud, estrellas, badges) vía `DCEpisodiosRequisitos.construirCartasEnriquecidasDesdeEntradas` (con `asegurarSkinsCargados` en previa).
  ...
  - **Cartas requeridas (jugador):** todas las entradas únicas de `cartas_jugador` en los bloques `combate` del timeline deben estar en la colección del usuario (copia base parent, misma regla que colección — `esCopiaBaseParentEnColeccion` en `cartas.js`). Módulo `js/episodiosRequisitos.js`; UI en panel del carrusel (`episodios.js`: sub-panel «Cartas requeridas» con **`DCCarrusel3d` + clase `evento-enemigos-carrusel`** — mismo estilo que enemigos en eventos/desafíos, vía `jugarPartida.css`; badge ✓ verde solo en la carta al frente). Al pulsar **Comenzar** sin cumplir requisitos: mensaje con nombres faltantes (`mensajeFaltantes`, incluye apariencias `Parent[n]`). Validación también en `DCEpisodioEngine.iniciarEpisodio` / `lanzarCombate`.
  - `recompensa`: `monedas`, `objetos[]`, `cartas[]`, `skins[]`. En **`cartas[]` puede usarse `Parent[skin_id]`** (p. ej. `Cyborg[0]`): `episodio-engine.js` → `aplicarRecompensas` desbloquea la apariencia (`persistirSkinsRecompensaEnUsuario`) y el panel muestra slot visual (`crearElementoRecompensaSkin`); entradas sin `[id]` siguen otorgando carta parent nivel 1. `skins[]` sigue aceptando IDs numéricos.
...
**Módulos:** `lib/editorXlsxSeguro.js` (escritura segura servidor), `lib/renderDeployMonitor.js` (versión/commit desplegado + consulta prod), `lib/editorGitPush.js` (servidor), `public/js/editorGuardadoSeguro.js` (confirmaciones cliente), `public/js/editorDeployMonitor.js` (espera deploy Render en UI), `public/js/editorGitPush.js` (modal push dev), `public/js/editorDevNav.js` (nav + guardias), `public/js/editorSessionLog.js` (registro sesión), `public/js/despliegue.js`, `public/js/editorCartaPicker.js`, …
...
- Git push editores (compartido):
  ...
  - `GET /api/public/deploy-version` → `{ commit, commitCorto, rama, url, esRender, ts }` (sin auth; para detectar fin de deploy)
  - `GET /api/editors/deploy/config` → `{ urlDev, urlProd, intervaloMs, timeoutMs, monitorDevDisponible, monitorProdDisponible, … }`
  - `GET /api/editors/deploy/version?destino=dev|prod` — versión en ejecución (prod vía proxy server-side a `RENDER_PROD_PUBLIC_URL`)
  - `POST /api/editors/git-push/dev` body `{ mensaje?, token? }` — commit+push unificado; respuesta incluye `commitSha` para monitor
...
- `episodios.html` → `cartas`, `carrusel3d`, `episodiosCapitulos`, `episodiosProgreso`, `episodiosRequisitos`, `episodios.js`, `episodio-engine.js`.
